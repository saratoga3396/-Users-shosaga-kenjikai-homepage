export type DetailedNews = {
  date: string;
  title: string;
  body: string;
  linkLabel?: string;
  linkUrl?: string;
};

export const detailedNews: DetailedNews[] = [
  {
    date: "2026.8.7",
    title: "夏祭りを開催しました",
    body: "8月7日、夏祭りを開催しました。アトラクションでは、チアダンスクラブCHERISH様、あっぱあず様にご出演いただき、利用者様にも笑顔があふれる楽しいひと時となりました。ご協力いただいた両団体の皆様に、心より御礼申し上げます。当日の様子は、トップページのギャラリーや広報誌、公式Instagramでご覧いただけますので、ぜひご覧ください。",
  },
  {
    date: "2026.5.21",
    title: "野田小学校の子どもたちからお米をいただきました",
    body: "野田小学校6年生の皆さんが育てたお米を、心のこもった手作りのお手紙とともにいただきました。「とてもおいしくできました」「5/23(土)の運動会にぜひ見に来てください。ソーランも踊ります」という温かいメッセージに、利用者様にも笑顔があふれました。素敵な贈り物を本当にありがとうございました。",
  },
  {
    date: "2026.5.20",
    title: "春の昼食会を開催しました",
    body: "5月20日、春の昼食会を開催しました。ちらし寿司や山菜の天ぷらなど、季節を感じる特別なメニューを皆さんで楽しみました。当日の様子はトップページのギャラリーでご覧いただけますので、ぜひご覧ください。",
  },
  {
    date: "2026.4.15",
    title: "広報52号（2026春号）発行しました！",
    body: "広報52号を発行しました。今号では、新任の生活相談員のご紹介のほか、ひなまつりや春のミニ運動会（玉入れ・輪投げ・風船バレー）の様子をお届けします。また、手作りおやつ「きらずだんつ（おからもち）」作りや、春の庭仕事・花壇づくりの活動もご紹介しています。ぜひご覧ください。",
    linkLabel: "広報52号（PDF）はこちら",
    linkUrl: "https://drive.google.com/file/d/1uIOJITPGQhBkOg5T0Ak6SdVKUdyRR_ub/view?usp=sharing",
  },
  {
    date: "2026.2.1",
    title: "グループホームぬくもり外部評価結果について",
    body: "特定非営利活動法人　いわての保健福祉支援研究会様による外部評価が2026年1月21日に確定しました。評価結果の詳細は下記リンクよりご覧いただけます。",
    linkLabel: "外部評価結果はこちら（外部リンク）",
    linkUrl: "https://www.wam.go.jp/wamappl/hyoka/003hyoka/hyokekka.nsf/aOpen?OpenAgent&JNO=0393100128&SVC=0001096&BJN=00&OC=01",
  },
  {
    date: "2026.1.20",
    title: "広報51号（2026冬号）発行しました！",
    body: "広報51号（2026冬号）を発行しました。冬の行事や地域交流の様子、季節ならではの取り組みを掲載しています。PDFをダウンロードしてぜひご覧ください。",
    linkLabel: "広報51号（PDF）はこちら",
    linkUrl: "https://drive.google.com/file/d/1GrlU261AfTBJdO9dLB-HDljOuHttu27e/view?usp=sharing",
  },
  {
    date: "2025.12.11",
    title: "讀賣新聞様のオンラインサイトで掲載されました",
    body: "讀賣新聞様のオンラインサイトにて、記事が掲載されました。下記リンクより詳細をご覧いただけます。",
    linkLabel: "記事はこちら（讀賣新聞オンライン）",
    linkUrl: "https://www.yomiuri.co.jp/national/20251210-GYT1T00257/",
  },
  {
    date: "2025.10.20",
    title: "広報50号（2025秋号）発行しました！",
    body: "広報50号（2025秋号）を発行しました。秋の行事や地域交流の様子、季節ならではの取り組みを掲載しています。PDFをダウンロードしてぜひご覧ください。",
    linkLabel: "広報50号（PDF）はこちら",
    linkUrl: "https://drive.google.com/file/d/1145O5ZXlw1WZgN4QmXNZowJO4Fpic-hd/view?usp=sharing",
  },
  {
    date: "2025.08.21",
    title: "敬老会を開催します！",
    body: "今年の敬老会は9/24(水)に開催いたします。コロナ禍以降、長らく入所者様と職員のみで開催してきておりましたが、今年はご家族様や慰問の皆様をお招きする形で開催いたします。会食はせず式典のみの開催となりますが、入所者様と楽しいひと時をお楽しみいただきたいと思います。※入所者様お一人にご家族様お一人までのお招きとなります。",
  },
  {
    date: "2025.08.20",
    title: "面会されるご家族様へのお願い",
    body: "6月以降、入所者様の居室において面会していただいておりますが、無断で施設内を動画で撮影したり、食事制限がある入所者様に居室で差し入れを食べさせる行為が確認されております。施設にはさまざまな方が入所されており、動画の撮影やSNSへの無断アップロード行為は肖像権の侵害に当たります。また、入所者様の中には病気の影響による栄養制限がある方や、食物を噛む力・飲み込む力が衰えている方も多くいらっしゃいます。入所者様の生命や健康を守り、入所者様及び職員のプライバシーを守るため、今一度面会ルールをご確認いただき、ご協力いただきますようお願い申し上げます。",
    linkLabel: "面会時の注意事項",
    linkUrl: "https://drive.google.com/file/d/1bSOamKiYkZvq8_M7MiLbte417qQmraGK/view?usp=sharing",
  },
  {
    date: "2025.07.19",
    title: "広報49号（2025夏号）発行しました！",
    body: "広報49号（2025夏号）を発行しました。地域交流やイベントの様子、季節のトピックを掲載しています。PDFをダウンロードしてぜひご覧ください。",
    linkLabel: "広報49号（PDF）はこちら",
    linkUrl: "https://drive.google.com/file/d/1_r_KyVZTCsnVRWYyli_h__271E4UuEW_/view?usp=sharing",
  },
  {
    date: "2025.06.15",
    title: "面会・外出・外泊制限を緩和します！",
    body: "長らく感染拡大防止のために実施していた面会・外出・外泊制限を一部緩和します。面会はエントランスから利用者様の居室に変更となり、外出や外泊も解禁となります。事前予約など一定のルールがありますので、詳しくは下記担当までご予約をお願いします。結の里担当：長内（おさない） 0194-78-3290 / ぬくもり担当：嵯峨（さが） 0194-78-3296。",
    linkLabel: "面会・外出・外泊のルール",
    linkUrl: "https://drive.google.com/file/d/17E6xjyxrkApSXSkQNa0s8Vc0awZiPfHn/view?usp=sharing",
  },
];
